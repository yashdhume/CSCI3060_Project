...
std::string BidTransaction::toString() {
    return "04 " + IOHelper::leftJustify(item.name,' ', 19) + " " + IOHelper::leftJustify(sellerUsername, ' ', 15) + " " + IOHelper::leftJustify(buyerAccount.username, ' ', 15) + IOHelper::rightJustify(std::to_string((int)item.currentBid), '0', 6);
}